...
class Database_Output(tk.Frame):
    ...
    def create_widgets(self, fields):
        ...
        for i, field in enumerate(fields):
            tk.Label(self, text=field, relief=tk.RIDGE, width=12, font='Helvetica 10 bold').grid(row=0, column=i)
        
        # create a separator line
        tk.Frame(self, height=2, bd=1, relief=tk.SUNKEN).grid(row=1, columnspan=len(fields))
    ...
    def update(self, data):
        ...
        for i, row in enumerate(data):
            for j, item in enumerate(row):
                tk.Label(self, text=item, relief=tk.RIDGE, width=12, font='Helvetica 10').grid(row=i+2, column=j)
